...
(function () {
  ...
  function initLoginLogic() {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        if (!res.ok) { showLoginError('Server error'); return; }
        const list = await res.json();
        const admin = (Array.isArray(list) ? list : []).find(a => String(a.adminId) === String(idInput) || String(a.id) === String(idInput));
        if (!admin) return showLoginError('Invalid credentials');

        // Development: plaintext password check (your db has 'pass')
        if (admin.pass && admin.pass === passInput) {
          sessionStorage.setItem('adminResourceId', String(admin.id ?? admin._id ?? admin.adminId));
          sessionStorage.setItem('adminId', String(admin.id ?? admin._id ?? admin.adminId));
          sessionStorage.setItem('adminKey', String(admin.adminId ?? admin.id));
          window.location.href = (admin.role === 'SYSTEM') ? 'system-admin.html' : 'admin-home.html';
          return;
        }

        if (admin.passHash) {
          // When passHash exists, use proper backend auth (not implemented here)
          showLoginError('Server requires secure login. Use backend auth.');
          return;
        }
  ...
  if (txt.includes('delete')) {
    ...
    if (announceBtn) {
      announceBtn.addEventListener('click', async (e) => {
        ...
        try {
          const r = await fetch(`${API_URL}/publicAnnouncements`, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ text, date: new Date().toLocaleDateString(), type: 'Update' }) });
  ...
  async function refreshPhaseTable() {
    ...
    if (!tbody) return;
    try {
      const r = await fetch(`${API_URL}/phases`);
      if (!r.ok) return;
      ...
      tbody.querySelectorAll('button.toggle-phase').forEach(b => b.addEventListener('click', async () => {
        ...
        try {
          const r = await fetch(`${API_URL}/phases/${id}`, { method:'PATCH', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ status: next }) });
      ...
      tbody.querySelectorAll('button.delete-phase').forEach(b => b.addEventListener('click', async () => {
        ...
        try {
          const r = await fetch(`${API_URL}/phases/${id}`, { method:'DELETE' });
  ...
  async function loadApplications() {
    ...
    try {
      let apps = [];
      let r = await fetch(`${API_URL}/applications`);
  ...
  async function updateApplicationDocStatus(appId, docStatus) {
    try {
      const r = await fetch(`${API_URL}/applications/${appId}`, { method:'PATCH', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ docStatus }) });
  ...
  function initRoomManagement() {
    ...
    if (deleteBtn) {
      deleteBtn.addEventListener('click', async () => {
        ...
        try {
          for (const id of selected) { try { await fetch(`${API_URL}/rooms/${id}`, { method:'DELETE' }); } catch (e) { /* ignore */ } }
        } catch (e) {}
  ...
  async function createRoomFromModal() {
    ...
    try {
      const r = await fetch(`${API_URL}/rooms`, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(newRoom) });
  ...
  async function saveEditedRoom() {
    ...
    if (!roomNo || !block || !capacity) return notify('Error','Fill required fields');
    try {
      const r = await fetch(`${API_URL}/rooms/${id}`, { method:'PATCH', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ block, roomNo, capacity, status }) });
